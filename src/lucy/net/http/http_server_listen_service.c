...
static int
l_http_server_listen_service_proc(l_service* self, l_message* msg)
{
  l_connind_message* connind = (l_connind_message*)msg;
  l_http_server_listen_service* ss = (l_http_server_listen_service*)self;
  ...
  default:
    if (msg->type != L_MESSAGE_CONNIND || l_filedesc_isEmpty(l_msg_getfd(msg))) {
      l_logw_2("http_server_service msg %d fd %d", ld(msg->msgid), ld(l_msg_getfd(msg)));
      return L_STATUS_EINVAL;
    }
    break;
  }

  return l_http_server_receive_conn(ss, connind);
}

static int
l_http_server_listen_service_getip(void* self, l_strt str)
{
  l_http_server_listen_service* ss = (l_http_server_listen_service*) self;
...
static int
l_http_server_listen_service_getmod(void* self, l_strt str)
{
  l_http_server_listen_service* ss = (l_http_server_listen_service*) self;
  if (l_strt_isEmpty(&str)) return false;
...
L_EXTERN int
l_http_startServer(const void* http_conf_name, int (*client_request_handler)(l_service*)) {
  l_sockaddr sa;
  l_http_server_listen_service* ss = 0;
  lua_State* L = l_get_luastate();

  ss = L_SERVICE_CREATE(l_http_server_listen_service);

  if (!http_conf_name) {
    http_conf_name = "http_default";
  }

  if (!l_luaconf_strv(L, l_http_server_listen_service_getip, ss, 2, http_conf_name, "ip")) {
  ...
  if (!l_luaconf_strv(L, l_http_server_listen_service_getmod, ss, 2, http_conf_name, "lua_module")) {
